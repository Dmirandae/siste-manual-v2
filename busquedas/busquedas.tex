...
\chapter{B\'usquedas mediante parsimonia} % (fold)
\label{cha:parsimonia}
\index{Parsimonia!b\'usquedas}
...
La forma m\'as sencilla de elaborar cladogramas es usando el algoritmo de Wagner: como el orden de entrada de los taxa afecta la topolog\'ia,  se realiza una aleatorizaci\'on de tal secuencia de entrada, % {\color{red} (FALTA Dayo, 1969)},  
...
\begin{itemize}
  \item Con \Pname{NONA/Winclada,  TNT,  POY} y \Pname{PAUP*}
	...
	\begin{enumerate}
	  \item  {B\'usquedas por omisi\'on}
        ...
        \pregunta{%{Preguntas}\\
          \begin{itemize}
    		    ...
    		    \item ?`C\'al es la b\'usqueda por omisi\'on en cada programa utilizado?
    	   \end{itemize}
        }
	  ...
	  \item{B\'usquedas modificadas}
		...
		Con el archivo el datos \RDatos{datosChica.dat} realice las siguientes b\'usquedas, modificando los comandos que sean necesarios:
  ...
  \pregunta{
    \begin{itemize}
		  ...
		  \item ?`Encontr\'o alguna tendencia en t\'erminos de tiempos o costos,  al aumentar el n\'umero de r\'eplicas?
	   \end{itemize}
  }
...
\cite{Goloboff1999}.
  ...
  \pregunta{
	 \begin{itemize}
		  \item ?`Cu\'al es y qu\'e hace el comando que permite hacer b\'usquedas con la t\'ecnica RATCHET en cada programa?
		  \item ?`Cada b\'usqueda o r\'eplica es independiente de otra?
		  \item ?`Hay alg\'un efecto en el resultado al hacer varias b\'usquedas continuas con el mismo n\'umero de r\'eplicas o es suficiente una sola b\'usqueda con m\'ultiples r\'eplicas? Realice b\'usquedas adicionales con par\'ametros diferentes que le permitan responder estas preguntas. Utilice un manejador gr\'afico donde pueda visualizar la tendencia de los datos obtenidos.
	 \end{itemize}
   }
...
\item An\'alisis Filogen\'etico en \Pname{R}: bibliotecas y dependencias
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \Cmdi{
     \begin{itemize}[label=$>$]
      \item primates $<$ $-$ read.phyDat ($"$chars2.txt$"$, 
...
\pregunta{?`Qu\'e otras funciones en otras bibliotecas permiten leer y/o escribir archivos tipo Nexus,  Fasta,  Phylip,  Clustal,  Sequential e Interleave?
}
...
\pregunta{
\begin{itemize}
\item ?`En qu\'e difiere cada \'arbol obtenido?
\item ?`En qu\'e consiste el m\'etodo de FICHT y el m\'etodo de SANKOFF?
\item  ?`Qu\'e hacen los algoritmos de agrupamiento UPGMA,  WPGMA,  NJ y UNJ?
\item ?`En qu\'e consiste el m\'etodo de construcci\'on de \'arboles de random.addition?
\end{itemize}
}

\item{Parsimonia y optimizaci\'on}
 
A partir de la matriz de datos \RDatos{primates} y las topolog\'ias construidas,  calcule el costo  de los \'arboles y obtenga el \'arbol de menor costo o score.
\Cmdi{
  \begin{itemize}[label=$>$]
...
La funci\'on \Rfunc{parsimony()} permite obtener el \'arbol de menor costo utilizando el algoritmo del m\'etodo SANKOFF o de FITCH,  en este caso es una b\'usqueda por omisi\'on dado que no se especifican los argumentos.
...
La funci\'on \Rfunc{optim.parsimony()} intenta encontrar el o los \'arboles m\'as parsimoniosos utilizando los m\'etodos de rearreglos NNI y SPR.

\pregunta{
\begin{itemize}
\item ?`En qu\'e difieren los m\'etodos de rearreglos NNI,  SPR y TBR?
\item ?`Cu\'ales son los argumentos por omisi\'on de las funciones \Rfunc{parsimony()} y \Rfunc{optim.parsimony()}?
\end{itemize}
}
...
\Rfunc{pratchet()} hace b\'usquedas usando el m\'etodo de Ratchet, estas b\'usquedas son hechas a partir de un \'arbol inicial ya sea optimizado o no,  aunque es preferible partir de un \'optimo ya dado. Tambi\'en puede iniciar haciendo una b\'usqueda para obtener el \'arbol o los \'arboles de partida, aplicar el m\'etodo de ratchet y \'optimizar.

\pregunta{
  \begin{itemize}
    \item ?`Hay diferencias entre las b\'usquedas con ratchet en t\'erminos de costos o tiempo?
    \item Escriba y ejecute la l\'inea de c\'odigo que le permitir\'ia realizar una b\'usqueda con: 70 iteraciones en Ratchet, Metodo SANKOFF, rearreglo SPR, SIN especificar el \'arbol inicial.
  \end{itemize} 
}
...
\preguntaGral{
\begin{itemize}
\item De los diferentes programas usados,  ?`Cu\'al estima usted que es el \'optimo? Explique las razones de su selecci\'on.
\item ?`Cu\'al cree usted que ser\'ia(n) el(los) criterio(s) para seleccionar entre los diferentes programas?
\item Elabore una tabla usando sus resultados y los de sus compa\~neros.  
\item Para cada matriz:
  \begin{itemize}
   \item ?`En qu\'e clase de b\'usqueda se obtuvo el mejor resultado?
   \item ?`Cu\'anto demor\'o para obtener dicho resultado?
  \end{itemize} 
...
Un \'ultima recomendaci\'on, en este laboratorio solo se utilizaron algunos tipos de b\'usquedas posibles y solo algunos comandos para cada programa. Revise otros comandos de b\'usqueda en estos programas u otros par\'ametros para los comandos usados en la pr\'actica.
