...
\section*{T\'ecnicas} Los m\'etodos para buscar y seleccionar \'arboles 
...
\item{en PhyML}
  ...
  \Pname{RaxML-NGy IQTree} son programas mucho m\'as r\'apido que 
  \Pname{PhyML} y por lo tanto deber\'ian ser una de sus primeras 
  ...
  \http{https://github.com/amkozlov/raxml-ng}.
  ...
  \item{en IQTree}
	...
	\begin{enumerate}
		...
		\Cmd{iqtree.exe -s example.phy -m TEST -b 10}
  ...
  \item{en RaxML-NG}
	...
	\begin{enumerate}
		\item Haga una corrida por defecto en \Pname{RaxML-NG}.
		\begin{enumerate}
			...
			\Cmd{ raxml-ng --msa dna.phy --model GTR+G --prefix ML0 --threads 2 --seed 2 }
			...
			\Datos{dna.phy} y con un prefijo de salida de ML0, usando 
			el modelo GTR + GAMMA; si desea usar un n\'umero distinto 
			de \'arboles de inicio, puede modificarlos:
			
			\Cmd{ raxml-ng --msa dna.phy --model GTR+G --prefix ML1 --threads 2 --seed 2 --tree pars\{25\},rand\{25\}}

		\end{enumerate}

		\item Haga una corrida en \Pname{RaxML}, para un an\'alisis particionado.
		    ...
		    \begin{enumerate}
			  ...
			  %~ %% version vieja\Cmd{raxml -m GTRGAMMA -p 12345 -q particionSimpleDNA.txt -s dna.phy -n ML01 }
			  
			  \Cmd{raxml-ng --msa dna.phy --model particionSimpleDNA.txt --prefix ML3 --threads 2 --seed 2 }\footnote{
			  particionSimpleDNA.txt\\
			  GTR+G+FO, gen1=1-1500\\
              GTR+G+FO, gen2=1501-7204}
			  
			  
			  donde los l\'imites de cada partici\'on se encuentran en 
			  el archivo particionSimpleDNA.txt, en este archivo debe 
			  indicar el tipo de datos que tiene, por ejemplo: DNA o 
			  BIN (morfol\'ogico/binario).

			  %~ \item si prefiere hacer el an\'alisis particionado pero 
			  %~ estimando las frecuencias de bases para cada partici\'on 
			  %~ de manera independiente debe usar:
